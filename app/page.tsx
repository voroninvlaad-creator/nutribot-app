...
const translations = {
  ru: {
    ...
    dictatePrompt: "Назовите блюдо и примерный вес",
    dictatePlaceholder: "Например: 200г гречки и куриное филе",
    ...
    searchPlaceholder: "Поиск по базе продуктов...",
    ...
    needMoreData: "Добавьте еще минимум одну запись для графика",
    history: "История взвешиваний",
    start: "Старт",
    inSystemSince: "Участник NutriBot",
    subsLevels: "Тарифные планы",
    current: "Текущий",
    free: "Бесплатно",
    allFeatures: "Все возможности",
    hideDetails: "Скрыть подробности",
    buySilver: "Перейти на Silver — 199 ₽",
    buyGold: "Активировать Gold — 499 ₽",
    yourTier: "Ваш тариф активен",
    proActive: "PRO-доступ активирован",
    accountSetup: "Настройка профиля NutriBot",
    ...
    reqSubDesc: "Эта функция доступна в расширенной версии. Разблокируйте в профиле.",
  ...
  en: {
    ...
    from: "of",
    kcal: "kcal",
    aiDietitian: "AI Dietitian: Suggest meal",
    proteins: "Protein",
    fats: "Fats",
    carbs: "Carbs",
    g: "g",
    ...
    dictatePrompt: "Say food and weight",
    dictatePlaceholder: "e.g., 200g oatmeal with banana",
    ...
    needMoreData: "Add at least two entries to display chart",
    ...
    buySilver: "Upgrade to Silver — 199 ₽",
    buyGold: "Activate Gold — 499 ₽",
    ...
    reqSubDesc: "This feature requires an upgraded tier. Unlock it in profile.",
...
const Dashboard = React.memo(({ current, goals, meals, onAddClick, selectedDate, setSelectedDate, requestAddMeal, currentWater, addWater, deleteMeal, checkAccess }) => {
  ...
  return (
    <div className="p-4 space-y-5 animate-in fade-in pb-28">
      ...
      {/* Плавающая кнопка записи голоса */}
      <button 
      ...
      {/* Модальное окно распознавания речи */}
      {isVoiceOpen && (
...
const CameraScanner = React.memo(({ onSave, onCancel, subscription, scansToday, incrementScan, checkAccess }) => {
  ...
  const handleProcessImage = (e) => {
    ...
    // Сжатие изображения через Canvas до 500px для моментальной обработки
      ...
      <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans max-w-md mx-auto shadow-2xl relative overflow-hidden select-none">
        ...
        <nav className="absolute bottom-0 w-full bg-slate-900/95 backdrop-blur-lg border-t border-white/5 py-2 z-30">
          <div className="flex justify-between items-center px-4">
            ...
            {/* Центральная кнопка быстрого сканирования */}
        ...
        {/* Модальное окно запроса подписки */}
        {upgradePrompt.show && (
