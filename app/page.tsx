...
const Dashboard = React.memo(({ current, goals, meals, onAddClick, selectedDate, setSelectedDate, requestAddMeal, currentWater, addWater, deleteMeal, checkAccess }) => {
  ...
  const calsGoal = goals?.calories || 2150;
...
const FoodSearch = React.memo(({ customFoods, saveCustomRecipeToDB, recentFoods, setRecentFoods, onSave, checkAccess, subscription, barcodeScansToday, incrementScan }) => {
  ...
  const combinedCatalog = useMemo(() => activeSubTab === 'global' ? MOCK_CATALOG : customFoods, [activeSubTab, customFoods]);
  const filteredFoods = useMemo(() => query.trim() ? combinedCatalog.filter(f => f.name.toLowerCase().includes(query.toLowerCase())) : combinedCatalog, [combinedCatalog, query]);
      ...
      {!selectedFood && (
        <>
          <div className="flex bg-slate-800/80 p-1.5 rounded-2xl border border-white/5">
            <button onClick={() => setActiveSubTab('global')} className={`btn-glass flex-1 py-2.5 text-xs font-bold rounded-xl transition-colors ${activeSubTab === 'global' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400'}`}>{t.base}</button>
            <button onClick={() => setActiveSubTab('custom')} className={`btn-glass flex-1 py-2.5 text-xs font-bold rounded-xl transition-colors ${activeSubTab === 'custom' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400'}`}>{t.myRecipes}</button>
          </div>
...
const WeightTracker = React.memo(({ history, onAdd }) => {
  ...
  const points = chartData.map((d, i) => `${(i / Math.max(chartData.length - 1, 1)) * 300},${100 - ((d.weight - minW) / range) * 90}`).join(' ');
...
const UserProfile = React.memo(({ currentSub, setSubscription }) => {
  ...
  const bronzeDetails = [
    ...
    { text: "Конструктор личных блюд и рецептов", active: true },
  ...
  return (
    <div className="p-4 space-y-5 animate-in fade-in pb-28">
      <div className="bg-slate-800/80 backdrop-blur-md rounded-3xl p-5 border border-white/5 flex items-center gap-4 shadow-lg">
        <div className="w-14 h-14 bg-gradient-to-tr from-emerald-500 to-teal-400 rounded-2xl flex items-center justify-center text-slate-950 font-black text-xl shadow-lg shadow-emerald-500/20"><User size={28} /></div>
    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 font-sans max-w-md mx-auto p-5 justify-center">
      ...
      <div className="space-y-4 bg-slate-900/90 p-5 rounded-3xl border border-white/5 shadow-2xl">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          ...
          <div><label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">{t.weight}</label><input type="number" value={form.weight} onChange={e => setForm({ ...form, weight: e.target.value })} className="w-full bg-slate-800 border border-white/5 rounded-xl py-2.5 text-center font-bold text-sm text-white outline-none focus:border-emerald-500"/></div>
        </div>
        <div>
...
export default function App() {
  ...
  const streakStyle = useMemo(() => {
    ...
    if (streakDays >= 30) return { text: "text-purple-400", fill: "fill-purple-400", bg: "bg-purple-500", border: "border-purple-500/40", grad: "from-purple-500 to-fuchsia-500" };
    return { text: "text-orange-400", fill: "fill-orange-400", bg: "bg-orange-500", border: "border-orange-500/40", grad: "from-orange-500 to-amber-500" };
  }, [streakDays]);

  const formattedSelectedDate = useMemo(() => `${selectedDate.getFullYear()}-${selectedDate.getMonth() + 1}-${selectedDate.getDate()}`, [selectedDate]);
  ...
  const current = useMemo(() => currentDayMeals.reduce(
    (acc, m) => ({ calories: acc.calories + (m.total?.calories || 0), protein: acc.protein + (m.total?.protein || 0), fat: acc.fat + (m.total?.fat || 0), carbs: acc.carbs + (m.total?.carbs || 0) }),
  ...
  const confirmAddMeal = useCallback((type) => {
    ...
    const newMeal = { ...pendingMeal, type, date: formattedSelectedDate, id: Date.now() + Math.random() };
      ...
      <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans max-w-md mx-auto shadow-2xl relative overflow-hidden select-none">
        ...
        <header className="px-4 py-3.5 bg-slate-900/90 backdrop-blur-md border-b border-white/5 flex justify-between items-center z-20">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button onClick={() => setActiveTab('profile')} className={`btn-glass text-xs font-bold border px-3 py-1.5 rounded-full flex items-center gap-1.5 ${subscription === 'gold' ? 'bg-amber-500/15 border-amber-500/40 text-amber-300 shadow-sm shadow-amber-500/20' : subscription === 'silver' ? 'bg-cyan-500/15 border-cyan-500/40 text-cyan-300' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
        ...
        <nav className="absolute bottom-0 w-full bg-slate-900/95 backdrop-blur-lg border-t border-white/5 py-2 z-30">
          <div className="flex justify-between items-center px-4">
            ...
            <div className="relative -top-5">
              <button onClick={() => checkAccess('silver') && setActiveTab('camera')} className="btn-glass w-14 h-14 bg-gradient-to-tr from-emerald-500 to-teal-400 text-slate-950 rounded-full flex items-center justify-center shadow-xl shadow-emerald-500/40 border-4 border-slate-950">
            ...
            <div className="bg-slate-900 w-full max-w-sm rounded-3xl p-5 border border-white/10 shadow-2xl animate-in slide-in-from-bottom">
              ...
              <div className="grid grid-cols-2 gap-2.5 mb-3">
                {['breakfast', 'lunch', 'dinner', 'snack'].map(type => (
                  <button key={type} onClick={() => confirmAddMeal(type)} className="btn-glass bg-slate-800 hover:bg-slate-700/80 p-3 rounded-2xl text-xs font-bold text-slate-200 border border-white/5 flex flex-col items-center gap-1">
                    <span className="text-xl">{type === 'breakfast' ? '🌅' : type === 'lunch' ? '☀️' : type === 'dinner' ? '🌙' : '🍎'}</span>
                    <span>{t[type]}</span>
                  </button>
                ))}
              </div>
              <button onClick={() => setPendingMeal(null)} className="btn-glass w-full py-3 bg-slate-800/60 text-slate-400 text-xs font-semibold rounded-xl">{t.cancel}</button>
            </div>
          </div>
        )}
        ...
        {upgradePrompt.show && (
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 w-full max-w-xs rounded-3xl p-6 border border-white/10 text-center shadow-2xl">
              <div className="w-16 h-16 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center mx-auto mb-3"><Crown size={32} /></div>
